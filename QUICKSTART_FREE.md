# Free VM quickstart — retired for Code17

This legacy guide is intentionally non-executable. Its old `rsync`/`rm -rf`
sequence could erase the only production `.env` and rollback evidence before
...
Use [`docs/FREE_DEPLOY.md`](docs/FREE_DEPLOY.md) for a fresh Oracle/Ubuntu VM,
starting from a clean, exact reviewed release commit. For every existing
installation:

1. preserve `/opt/d-company-erp`, its mode-`0600` `.env`, running immutable
   images, database volumes, and rollback evidence;
2. schedule a write outage and sync every tablet outbox;
...
login. Never delete/replace the live project directory, regenerate an existing
env, or run a direct Compose rebuild as an upgrade.
