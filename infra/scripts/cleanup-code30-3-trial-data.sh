...
SQL_FILE="$SCRIPT_DIR/cleanup-code30-3-trial-data.sql"
RUNNER_FILE="$SCRIPT_DIR/$(basename -- "${BASH_SOURCE[0]}")"
CONFIRMATION=APPLY_CODE30_3_VERIFIED_TRIAL_CLEANUP
...
usage() {
  cat <<'USAGE'
Usage:
  cleanup-code30-3-trial-data.sh --postgres-container NAME

  cleanup-code30-3-trial-data.sh --postgres-container NAME \
    --backend-container NAME \
    --apply \
    --confirm APPLY_CODE30_3_VERIFIED_TRIAL_CLEANUP \
...
database inside the same PostgreSQL container, verifies it has the live
database migration, and runs the cleanup dry run on the restore.  Its complete database fingerprint
...
Run it after the Code30.3 deployment, with the backend stopped, so the
versioned replay fence is already live when tablets reconnect.  The disposable
restore database is dropped on every exit.
USAGE
}
...
cleanup_restore() {
  ...
  if [[ -n "$restore_db" ]]; then
    ...
    ' cleanup-code30-3 "$restore_db" >/dev/null; then
...
run_cleanup_sql() {
  ...
  docker exec -i "$postgres_container" sh -eu -c '
    ...
    exec psql -X --no-psqlrc --set=ON_ERROR_STOP=1 --quiet --tuples-only --no-align \
      ...
      --set="backup_sha256=$4" \
      --set="source_git_sha=$5" \
      --set="executor_name=$6"
  ' cleanup-code30-3 "$database" "$apply_value" "$expected_fingerprint" \
    "$backup_sha256" "$source_git_sha" "$executor_name" < "$SQL_FILE"
}
...
if [[ "$apply" == true ]]; then
  ...
  [[ "$runner_canonical" == "$repo_root/infra/scripts/cleanup-code30-3-trial-data.sh" ]] || \
    die "cleanup runner is not the canonical tracked repository script"
  [[ "$sql_canonical" == "$repo_root/infra/scripts/cleanup-code30-3-trial-data.sql" ]] || \
  ...
  for tracked_path in \
    infra/scripts/cleanup-code30-3-trial-data.sh \
    infra/scripts/cleanup-code30-3-trial-data.sql
  do
  ...
  ' cleanup-code30-3 "$restore_db_candidate"
  ...
  docker exec -i "$postgres_container" sh -eu -c '
    ...
    exec pg_restore --exit-on-error --no-owner --no-privileges \
      --username "$POSTGRES_USER" --dbname "$1"
  ' cleanup-code30-3 "$restore_db" < "$backup_file"
  ...
  restored_revision=$(read_revision "$restore_db")
  live_revision=$(read_revision "$production_db")
  [[ "$restored_revision" == 0082 && "$live_revision" == 0082 ]] || \
    die "restored backup and live database must both be at migration 0082 (Code30.3 deployed first)"
  ...
  docker exec "$postgres_container" sh -eu -c '
    : "${POSTGRES_USER:?POSTGRES_USER is not set}"
    exec dropdb --username "$POSTGRES_USER" --if-exists --force "$1"
  ' cleanup-code30-3 "$restore_db" >/dev/null
  restore_db=
