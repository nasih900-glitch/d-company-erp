-- Restore-only rehearsal for an owner-reviewed delete-or-retain Code30.3 decision.
-- This file has no apply mode and always rolls back.
...
DO $$
BEGIN
    IF current_database() !~ '^code30_combined_restore_[0-9]+_[0-9]+$' THEN
        RAISE EXCEPTION 'combined cleanup rehearsal is restricted to its disposable restore database';
    END IF;
    IF NOT pg_try_advisory_xact_lock(hashtext('dcompany-code30.3-combined-cleanup-rehearsal')) THEN
        RAISE EXCEPTION 'another combined cleanup rehearsal holds the advisory lock';
    END IF;
END
$$;
...
INSERT INTO c3c_inputs VALUES (:'backup_sha256', :'maintenance_sql_sha256');
...
BEGIN
    ...
    INSERT INTO audit_log (
        ...
        NULL, 'rehearse-code30-3-combined-cleanup/1', (m->>'terminal_id')::uuid,
        m->>'cleanup_id', NULL, NULL, NULL, NULL, NULL, NULL,
        'Owner-reviewed combined Code30.3 trial-cohort maintenance'
