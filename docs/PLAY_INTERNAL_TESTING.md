...
This is the identity reserved for the next candidate. No final `3.0.5` APK/AAB
...
physical-device acceptance proof for `3.0.5`, and this preparation has not
...
- Android JVM tests, compilation, lint, assembly, and emulator instrumentation
  pass;
- the release manifest says package `cloud.dcompany.erp`, version `3.0.5`, code
  `6`, and the production HTTPS API above;
...
physical Redmi Pad proof. No such run has yet been completed for `3.0.5`.
...
3. Upload the signed AAB from the green repository release, normally named
   `d-company-erp-v3.0.5.aab`. Do not upload `app-debug.apk`.
4. Confirm Play reads package `cloud.dcompany.erp`, version `3.0.5`, and version
   code `6` from the bundle.
5. Use release name `3.0.5 (6)`.
...
If Play reports that version code `6` was already used, increment `versionCode`
...
4. Confirm Android reports version `3.0.5`.
