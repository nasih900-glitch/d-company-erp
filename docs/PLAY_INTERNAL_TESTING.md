...
Codes `16` (`3.1.5`) and `17` (`3.1.6`) are immutable predecessors. Code `18`
(`3.1.7`) is a local direct-channel visual candidate only. Do not upload it to
Play or register, stage, advertise, or activate it through the server without a
separate rollout review. Physical Redmi Pad 2 acceptance remains a separate
gate.
...
- the future bundle metadata says package `cloud.dcompany.erp`, the deliberately
  chosen new version/code above `18`, and the production HTTPS API above;
