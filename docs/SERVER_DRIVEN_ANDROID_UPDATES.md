...
Tag `v3.1.7` (version code `18`) failed its pre-signing release gate and produced
no authorised APK. The tag is immutable and must never be moved, rebuilt, or
reused. The corrected server-delivery candidate is `3.1.8` (version code `19`).
Only the exact signed artifact and release manifest produced together by the
green `v3.1.8` GitHub Actions workflow may be staged. A local Gradle build or
...
2. Coordinate the application at `3.1.8` / code `19`. Run the complete
...
3. Download both files without renaming or modifying either one. First run a
   ...
   python3 ops/stage_android_release.py \
     --manifest /secure/release-3.1.8/release-manifest.json \
     --apk /secure/release-3.1.8/d-company-erp-v3.1.8-direct.apk \
...
5. In the owner ERP release screen, compare version, release notes, SHA-256,
   size, signer and source evidence. Activate only the staged code-`19` row.
...
6. On one code-`14` tablet, refresh the update check, download, install and
   reopen code `19`. Verify sign-in, shift, Gaming, POS settlement, offline queue
