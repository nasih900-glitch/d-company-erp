...
| **DB migrations** | Alembic — current Code30.3 candidate head `0079` |
...
backend/
  ...
  alembic/versions/                  migrations chained through current head 0079
...
Code30.3 is the additive stale-Gaming-recovery patch at `v3.1.30`, Android
build `38`, Room `52`, and Alembic `0079`. Its immutable base is Code30.2
...
from that Git object and uses a separate Code30.3 map for current files.

Stale Gaming cleanup is exact-candidate only. The owner reviews station,
session, price, duration and hash in Web; the build-38 tablet must reconnect,
apply the matching directive and acknowledge it. Never add a broad clear-all,
direct SQL repair or remote Room edit. Changed evidence supersedes the prior
record and requires a new approval. This patch does not activate rewards,
WhatsApp or SMTP.
