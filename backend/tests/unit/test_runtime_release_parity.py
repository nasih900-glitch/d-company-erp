...
IDENTITY = {"version_name": "3.1.14", "source_git_sha": "ab" * 20}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            _response(body=json.dumps({"version_name": "3.1.14"}).encode()),
            "frontend_identity_invalid",
        ),
        (
        ...
        (_response(body=BODY[:-1] + b',"version_name":"3.1.14"}'), "frontend_identity_invalid"),
        (
...
async def test_production_lifespan_warms_before_serving_and_cancels_supervisor(
    ...
    async with application_main.lifespan(SimpleNamespace(version="3.1.14")):
        assert events == ["warm"]
...
def _release(status="staged") -> SimpleNamespace:
    ...
    return SimpleNamespace(
        ...
        version_code=25,
        **IDENTITY,
        update_url="https://erp.example.test/downloads/android/code25.apk",
        release_notes="Checked release",
        apk_sha256="cd" * 32,
        apk_size_bytes=100,
        apk_signing_cert_sha256="ef" * 32,
        manifest_sha256="01" * 32,
        source_release_ref="v3.1.14",
