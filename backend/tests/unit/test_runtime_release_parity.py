...
IDENTITY = {"version_name": "3.1.15", "source_git_sha": "ab" * 20}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            _response(body=json.dumps({"version_name": "3.1.15"}).encode()),
            "frontend_identity_invalid",
        ),
        (
        ...
        (_response(body=BODY[:-1] + b',"version_name":"3.1.15"}'), "frontend_identity_invalid"),
        (
...
async def test_production_lifespan_warms_before_serving_and_cancels_supervisor(
    ...
    async with application_main.lifespan(SimpleNamespace(version="3.1.15")):
        assert events == ["warm"]
